...
public class My_Profile extends JPanel implements ActionListener {
	int width,height;
...
//	JButton backbutton;
	...
	JLabel password,ipassword;
	JLabel personalimage;
	JLabel updateprofile,createchannel,creategroup;
	MainFrame mainFrame;
	
	
	

	public My_Profile(MainFrame mainFrame,String dusername,String dpassword) throws IOException {
//	public My_Profile(MainFrame mainFrame) throws IOException {
	...
	//	backbutton=new JButton(new ImageIcon(buttonicon));
		...
		specifylabel1(password,260 , 390);
		...
		ipassword=new JLabel();
		specifylabel(dusername,iusername,500,270);
		specifylabel("9361962568",iphonenumber,500,330);
		specifylabel(dpassword,ipassword,500,390);
		...
		createchannel=new JLabel("Create channel");
		creategroup=new JLabel("Create group");
		updateprofile=new JLabel("Update profile");
		specifylabel(createchannel, 20,200);
		specifylabel(creategroup,20,250);
		specifylabel(updateprofile,20,300);
		
		
		Font f = new Font("Dialog", Font.PLAIN, 27);
		name=new JLabel("Amirhossein Bavand");
	...
	private void specifylabel(JLabel temp, int posx,int posy) {
		...
		temp.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent arg0) {
				// TODO Auto-generated method stub
				if(arg0.getSource().equals(createchannel))
					System.out.println("salam");
				
				if(arg0.getSource().equals(creategroup))
					System.out.println("yes");
				if(arg0.getSource().equals(updateprofile))
					try {
						mainFrame.sms(7);
					} catch (IOException e) {
						// TODO Auto-generated catch block
